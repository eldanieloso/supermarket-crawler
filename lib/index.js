...
async function scrapeOne(page, url) {
  ...
  let element = await page.evaluate(() => {
    let links = Object.values(document.body.querySelectorAll('a.name')).map(x => {
      return x.href;
    });
    return {
...
async function scrape(url) {
  ...
  let element = await scrapeOne(page, url); //guardar

  console.log(element.title); // logging results

  if (element) {
    if (element.related) {
      for (const link of element.related) {
        let element = await scrapeOne(page, link); //guardar
